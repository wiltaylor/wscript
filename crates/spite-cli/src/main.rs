...
#[derive(Parser)]
#[command(name = "spite", about = "SpiteScript language runner and tooling")]
struct Cli {
    #[command(subcommand)]
    command: Option<Command>,

    /// Run a .spite file directly
    #[arg(value_name = "FILE")]
    file: Option<PathBuf>,
}
    ...
    match cli.command {
        ...
        None => {
            if let Some(file) = cli.file {
                ...
                eprintln!("Usage: spite <file> or spite <command>");
                eprintln!("Run 'spite --help' for more information.");
                std::process::exit(1);
            }
        }
    }
...
fn cmd_run(file: PathBuf, function: &str, debug: bool, fuel: Option<u64>) -> miette::Result<()> {
    ...
    let mut engine = spite_script::Engine::new();
...
fn cmd_check(file: PathBuf) -> miette::Result<()> {
    ...
    let engine = spite_script::Engine::new();
...
async fn cmd_lsp() -> miette::Result<()> {
    #[cfg(feature = "lsp")]
    {
        ...
        use spite_script::query_db::QueryDb;
        use spite_script::lsp::SpiteLspServer;
        use std::sync::Arc;
        use tokio::sync::RwLock;

        let bindings = Arc::new(spite_script::BindingRegistry::new());
        let db = Arc::new(RwLock::new(QueryDb::new(bindings)));

        let (service, socket) = LspService::new(|client| SpiteLspServer::new(client, db.clone()));
...
async fn cmd_dap(_port: u16) -> miette::Result<()> {
    #[cfg(feature = "dap")]
    {
        let engine = spite_script::Engine::new().debug_mode(true);
        let mut server = spite_script::dap::SpiteDapServer::new(engine, _port);
