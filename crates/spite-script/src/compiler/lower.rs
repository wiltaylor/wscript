...
//!
//! Converts the high-level SpiteScript AST into a flat, typed IR that maps
...
impl Lowerer {
    ...
    fn lower_let(&mut self, l: &ast::LetStmt, ctx: &mut FnCtx, out: &mut Vec<IrStmt>) {
        ...
        if is_lambda_init && self.lambda_counter > lambda_counter_before {
            if let Some(lambda_fn_name) = self.last_lambda_name() {
...
impl Lowerer {
            ...
            ExprKind::Pipe { lhs, rhs } => {
                ...
                match &rhs.kind {
                    ExprKind::Call { callee, args } => {
                        ...
                        match name.as_str() {
                            "filter" if args.len() == 1 => {
                                return self.lower_pipeline_filter(lhs, &args[0].value, ctx);
                            }
                            "map" if args.len() == 1 => {
                                return self.lower_pipeline_map(lhs, &args[0].value, ctx);
                            }
                            "for_each" if args.len() == 1 => {
                                return self.lower_pipeline_for_each(lhs, &args[0].value, ctx);
                            }
                            "collect" if args.is_empty() => {
                                return self.lower_expr(lhs, ctx);
                            }
                            "any" if args.len() == 1 => {
                                return self.lower_pipeline_any(lhs, &args[0].value, ctx);
                            }
                            "all" if args.len() == 1 => {
                                return self.lower_pipeline_all(lhs, &args[0].value, ctx);
                            }
                            "find" if args.len() == 1 => {
                                return self.lower_pipeline_find(lhs, &args[0].value, ctx);
                            }
                            "reduce" if args.len() == 1 => {
                                return self.lower_pipeline_reduce(lhs, &args[0].value, ctx);
                            }
                            "fold" if args.len() == 2 => {
                                return self.lower_pipeline_fold(lhs, &args[0].value, &args[1].value, ctx);
                            }
                            "take" if args.len() == 1 => {
                                return self.lower_pipeline_take(lhs, &args[0].value, ctx);
                            }
                            "skip" if args.len() == 1 => {
                                return self.lower_pipeline_skip(lhs, &args[0].value, ctx);
                            }
                            _ => {
    ...
    fn lower_pipeline_take(
        ...
        let mut stmts = Vec::new();
        stmts.push(IrStmt::Let { local: arr_local, ty: IrType::I32, value: Some(arr) });
        stmts.push(IrStmt::Let { local: n_local, ty: IrType::I32, value: Some(n) });
        stmts.push(IrStmt::Let {
            local: result_local,
            ty: IrType::I32,
            value: Some(IrExpr::HostCall {
